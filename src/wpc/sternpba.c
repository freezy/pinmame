...
//Static Variables
static char spa_dllpath[MAX_PATH];
...
struct {
	int vblankCount;
	INT16 samplebuf[SPA_SNDBUFSIZE];
	INT16 lastsamp;
	int sampout;
	int sampnum;
	int coindoor;
...
static uint8_t *spa_fbuffer, *spa_bbuffer, *persistbuf;
...
static int spa_sndbufferlength()
{
...
static void spa_sh_update(int num, INT16 *buffer, int length)
{
	if (length > 0)
	{
		int ii;
		...
		{
			spalocals.lastsamp = buffer[ii] = spalocals.samplebuf[spalocals.sampout];
...
static uint32_t dllcrc = 0;
static uint32_t imagecrc = 0;


// Some very messy references to memory locations within TPA release build DLL, mostly found
// with CheatEngine.

static const int hsptr[] = { 0x990, 0x9b8, 0xaf0, 0xb40, 0xb90, 0xa70 };
...
static MACHINE_INIT(spa) {
	...
	if (once)
	{
...
#endif
		GetModuleFileName(hInst, spa_dllpath, MAX_PATH);
		*strrchr(spa_dllpath, '\\') = 0;
		...
		sprintf_s(path, sizeof(path), "%s\\spagb100\\SternGB.dll", spa_dllpath);
		FILE* f = fopen(path, "rb");
		if (f == NULL) {
			char tmp[MAX_PATH];
			sprintf_s(tmp, sizeof(tmp), "Failed to open %s", path);
			critical_error(tmp);
			return;
		}
		fseek(f, 0, SEEK_END);
		...
		// Load the ROM
		sprintf_s(path, sizeof(path), "%s\\spagb100\\image.bin", spa_dllpath);
		f = fopen(path, "rb");
		if (f == NULL) {
			char tmp[MAX_PATH];
			sprintf_s(tmp, sizeof(tmp), "Failed to open %s", path);
...
static MACHINE_STOP(spa) {
	...
	char path[MAX_PATH];
	sprintf_s(path, sizeof(path), "%s\\spagb100\\gbnv.txt", spa_dllpath);
	FILE *f = fopen(path, "w");
	if (f)
	{
		if (dllcrc == DLLCRC_TPAGBDLL)
		...
		if (dllcrc == DLLCRC_TPAGBDLL)
		{
			...
			mem = (int*)ballspergameptr;
			fprintf(f, "BallsPerGame:%d\n", *mem);
			mem = (int*)profanityptr;
...
/*  VBLANK Section  */
/********************/
static /*INTERRUPT_GEN(spa_vblank)*/ void spa_vblank(int data)
{
	int i;
	...
	while (spa_sndbufferlength() < SPA_SNDBUFSIZE - 800)
	{
		...
		{
			spalocals.samplebuf[spalocals.sampnum] = audio[i]; //!! no stereo??
...
of the foreground are set.    SPA DLL appears to give us this same data. */
...
static PINMAME_VIDEO_UPDATE(spadmd_update) {
	...
	for (y = 0; y < 32; y++)
	{
		int offs = y * 128;
		int x;
		for (x = 0; x < 128; x++,offs++)
		{
			const UINT8 RAM1 = spa_fbuffer[offs];
			const UINT8 RAM2 = spa_bbuffer[offs];
...
/ Ghostbusters LE
/-------------------------------------------------------------------*/
INITSPAGAME(spagb, GEN_SPA, spa_dmd128x32, SAM_8COL, SPA_GAME_GB)

SPA_ROM32MB(spagb_100, "XXX.bin", CRC(00000000) SHA1(0000000000000000000000000000000000000000), 0x00000000)
SPA_INPUT_PORTS_START(spagb, 1)
CORE_GAMEDEF(spagb, 100, "Ghostbusters Limited Edition (V1.15)", 2013, "Stern", spa, 0)
